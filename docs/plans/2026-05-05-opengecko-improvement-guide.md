...
Known follow-ups:
- Move from OHLCV fallback observability to actual source-backed coverage expansion: broader documented target batches, provider preset replay breadth, sync/backfill reliability, and production freshness alerting.
...
| --- | --- | --- | --- | --- |
| 1 | Market chart production freshness alerting | Add summary counts and operator workflow for targets that pass first-run `coverage.freshness` but miss stricter production freshness | Public response shape changes, live provider calls in tests, automatic config writes, or changing existing `coverage.freshness` semantics | `/diagnostics/market_charts` makes production-stale targets visible at summary level and tests prove public chart/OHLC payloads stay unchanged |
| 2 | Market chart backfill verification breadth | Add offline fixture/replay coverage that verifies daily and 1m source rows produce chart and OHLC responses for every seeded starter target class | Live provider calls in tests, expanding beyond seeded starter targets, or changing response shapes | Fixture-backed sync tests prove representative daily and intraday target classes write source rows and public route tests prove source-backed precedence |
