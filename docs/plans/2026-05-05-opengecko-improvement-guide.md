...
| --- | --- | --- | --- | --- |
| 1 | Optional provider scheduler status in runtime diagnostics | Surface whether the optional scheduler is enabled, scheduled, interval length, and in-flight state in operational diagnostics without exposing provider URLs | Exposing provider credentials or changing public CoinGecko-compatible response shapes | Runtime diagnostics tests prove disabled/enabled scheduler status and keep `/diagnostics/jobs` as the detailed per-job view |
