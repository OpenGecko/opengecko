...
`/diagnostics/coverage_matrix` is the runtime source of truth for endpoint-family ownership classes. The route is generated by `src/services/coverage-matrix.ts`; docs-drift tests load that same builder and fail if this tracker’s seeded-validation counts, percentages, or per-family ownership rows drift by more than ±1 percentage point.

The current checked-in seeded validation matrix has 10 families:
...
| `historical_charts` | `seeded` | 3 |
| `supply_charts` | `fixture` | 2 |
| `treasury` | `fixture` | 3 |
| `stable_catalog` | `seeded` | 3 |

Source-capable providers remain CCXT, DeFiLlama, Subsquid/SQD-compatible replay, currency-api, and optional HTTP replay adapters. A family is not documented as `live` in the seeded validation matrix unless that matrix has source-backed rows, but production/runtime diagnostics can promote hot-market families after successful provider refreshes.

**Key gap**: "Route implemented" ≠ "has live data". The 76/76 parity claim refers to HTTP contract surface (routing, parameters, response structure), not data fidelity. Several families serve seeded, fixture, or hybrid data unless the configured runtime has fresh source-backed rows.

## Workstream Status

| Strategic workstream | Operational scope | Status | Notes |
| --- | --- | --- | --- |
| WS-A Compatibility fidelity | Parameter precedence, error shapes, serializers, divergence tracking | partial | Route coverage is broad and the compatibility audit records 77 / 77 active non-NFT parity-matrix endpoints as implemented, but current runtime regressions mean the practical release gate is not yet satisfied |
...
| `/simple/*` | R0 | done | seeded validation matrix: `simple=seeded`; runtime can promote to live | `/simple/supported_vs_currencies`, `/simple/price`, `/simple/token_price/{id}`, and `/exchange_rates` are implemented and tested; hot quote routes serve source-backed snapshots when market refresh writes rows from CCXT/DeFiLlama/currency providers |
...
| Core coin market endpoints | R1 | partial | seeded validation matrix: `coins_markets=seeded`, `coin_detail=seeded`; runtime can promote hot market fields | `/coins/markets` serves refreshed source-backed snapshots when market refresh is running; `/coins/{id}` includes CCXT-enriched description/links when available; history/chart/OHLC fidelity work remain pending, and sparklines still rely on seeded/synthetic history |
...
- `/simple/*` and `/coins/markets`: seeded validation matrix classes are `simple=seeded` and `coins_markets=seeded`, but production/runtime reads can be live when market refresh has written source-attributed CCXT/DeFiLlama/currency rows within the freshness budget.
...
- `/coins/{id}/market_chart`, `/ohlc`, `/ohlc/range`, `/history`: matrix class is `historical_charts=seeded` in the seeded validation baseline; routes can read source-attributed replay/live rows and canonical OHLCV where present, but matrix promotion now requires source-backed market-chart breadth/depth gates so seeded 7-day synthetic fallback and replay-only rows are not overclaimed as live.
