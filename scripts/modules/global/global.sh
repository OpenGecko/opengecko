...
check_json_expr "defi response exposes provenance classification" "/global/decentralized_finance_defi" '(.meta | type == "object") and (.meta | has("source") and has("state") and has("live") and has("unavailable") and has("degraded") and has("provider_ids") and has("latest_source_at") and has("reason_codes")) and (.meta.state as $state | ["live","degraded","unavailable"] | index($state) != null) and ((.meta.state == "live" and .meta.live == true and .meta.unavailable == false and .meta.degraded == false and (.meta.reason_codes | length) == 0) or (.meta.state != "live" and (.meta.reason_codes | length) > 0))' "defi metadata classifies source as live, degraded, or unavailable"
