...
check_json_expr "pool trades return source metadata and ordered trade rows" "/onchain/networks/${NETWORK_ID}/pools/${POOL_ADDRESS}/trades?limit=5" 'has("data") and (.data | type == "array") and (.data | length > 0) and ([.data[0] | .type == "trade" and (.attributes | has("tx_hash") and has("block_timestamp") and has("volume_in_usd"))] | all(.)) and (.meta | has("source") and has("fixture") and has("updated_at")) and ((.meta.fixture == true and .meta.source == "fixture") or (.meta.fixture == false and (.meta.source != "fixture") and (.meta.updated_at | type == "string")))' "pool trades fail unsafe fixture/live claims"
check_status "GET /onchain/networks/:network/tokens/:address/trades responds" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/trades?limit=5"
check_json_expr "token trades return source metadata and ordered trade rows" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/trades?limit=5" 'has("data") and (.data | type == "array") and (.data | length > 0) and ([.data[0] | .type == "trade" and (.attributes | has("tx_hash") and has("block_timestamp") and has("volume_in_usd"))] | all(.)) and (.meta | has("source") and has("fixture") and has("updated_at")) and ((.meta.fixture == true and .meta.source == "fixture") or (.meta.fixture == false and (.meta.source != "fixture") and (.meta.updated_at | type == "string")))' "token trades fail unsafe fixture/live claims"
check_status "GET top holders responds" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/top_holders"
check_json_expr "top holders remain out-of-scope/degraded" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/top_holders" 'has("meta") and (.meta.out_of_scope == true) and (.meta.degraded == true) and (.meta.scope | contains("out-of-scope"))' "top holders cannot be promoted as live-complete"
check_status "GET top traders responds" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/top_traders"
check_json_expr "top traders remain out-of-scope/degraded" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/top_traders" 'has("meta") and (.meta.out_of_scope == true) and (.meta.degraded == true) and (.meta.scope | contains("out-of-scope"))' "top traders cannot be promoted as live-complete"
check_status "GET holders chart responds" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/holders_chart"
check_json_expr "holders chart remains out-of-scope/degraded" "/onchain/networks/${NETWORK_ID}/tokens/${TOKEN_ADDRESS}/holders_chart" 'has("meta") and (.meta.out_of_scope == true) and (.meta.degraded == true) and (.meta.scope | contains("out-of-scope"))' "holders chart cannot be promoted as live-complete"
...
check_status "GET onchain megafilter responds" "/onchain/pools/megafilter?networks=${NETWORK_ID}"
check_json_expr "megafilter remains out-of-scope/degraded" "/onchain/pools/megafilter?networks=${NETWORK_ID}" 'has("meta") and (.meta.out_of_scope == true) and (.meta.degraded == true) and (.meta.scope | contains("out-of-scope"))' "megafilter cannot be promoted as live-complete"
