...
check_json_expr "exchange detail returns overview fields and ticker array" "/exchanges/${EXCHANGE_ID}" 'has("id") and has("name") and has("centralized") and (.tickers | type) == "array"' "exchange detail contains overview metadata and a contract-compatible ticker array"
...
check_json_expr "contract-address formatting rewrites USDC base when ticker rows are available" "/exchanges/${EXCHANGE_ID}?dex_pair_format=contract_address" '(.tickers | map(select(.coin_id == "usd-coin"))) as $usdc | ($usdc | length == 0) or ($usdc[0].base == "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48")' "usd-coin ticker base is rendered as a contract address when present"

module_section "Exchange Tickers"
check_status "GET /exchanges/:id/tickers responds" "/exchanges/${EXCHANGE_ID}/tickers"
check_json_expr "exchange tickers return market metadata and pair fields when rows are available" "/exchanges/${EXCHANGE_ID}/tickers" 'has("name") and (.tickers | type) == "array" and ([.tickers[] | has("base") and has("target") and has("market") and (.market | has("identifier") and has("name"))] | all(.))' "ticker payload includes market metadata and pair fields when populated"
check_json_expr "coin_ids filter narrows to the requested asset" "/exchanges/${EXCHANGE_ID}/tickers?coin_ids=ethereum&order=volume_asc" '(.tickers | length) == 0 or ([.tickers[].coin_id] | all(. == "ethereum"))' "coin_ids filter returns only ethereum rows when rows are available"
check_json_expr "depth=true adds depth-cost fields when rows are available" "/exchanges/${EXCHANGE_ID}/tickers?coin_ids=usd-coin&depth=true&dex_pair_format=contract_address" '(.tickers | length) == 0 or ([.tickers[] | has("cost_to_move_up_usd") and has("cost_to_move_down_usd") and .base == "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48"] | all(.))' "depth-enabled tickers include order-book cost fields when populated"
...
check_status "GET /derivatives/exchanges/:id/tickers responds" "/derivatives/exchanges/${DERIVATIVES_EXCHANGE_ID}/tickers"
check_json_expr "derivatives exchange ticker route exposes classified contract rows" "/derivatives/exchanges/${DERIVATIVES_EXCHANGE_ID}/tickers" '(.data | has("id") and has("name") and (.tickers | type) == "array" and (.tickers | length > 0) and ([.tickers[] | has("symbol") and has("contract_type") and has("trade_volume_24h_btc")] | all(.))) and (.meta | type == "object") and (.meta | has("fixture") and has("source_backed_tickers") and has("fallback_tickers"))' "derivatives ticker route reuses detail ticker rows and source/fallback metadata"
