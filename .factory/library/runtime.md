...
- The manifest-managed validation API on port `3102` should adopt that same seeded-bootstrap mode when it imports the same persisted rows. Validation should exercise the intended seeded-runtime semantics, not preserve the older stale/degraded-only contract on that path.
